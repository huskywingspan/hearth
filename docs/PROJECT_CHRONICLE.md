...
| 2026-02-11 | **Design Philosophy Established (§1.2)** | "Build it like Apple would." Hearth is a design challenge, not just engineering. The frontend is the product, not a skin over the backend. Emotional connection with the app is the marketing strategy — word-of-mouth is the only viable growth path. Five emotional design principles codified: Familiarity, Discovery, Seasonality, Shared Wonder, Nostalgia. Seed ideas for delight moments: interactive fireplace, seasonal ambience shifts, rain-on-window, welcome mat, anniversary moments, sound easter eggs. The test: "Would you text your friend to say 'dude, click the fireplace'?" |
| 2026-02-15 | **Competitive Intelligence: Stoat (née Revolt)** | Deep dive into Stoat.chat (formerly Revolt), an open-source Discord clone that rebranded Oct 2025 after a C&D. Key findings: Rust backend + Solid.js frontend, AGPL-3.0, **14 Docker containers** (MongoDB, Redis/KeyDB, RabbitMQ, MinIO, Caddy, API, Events/Bonfire, Web, Autumn, January, Gifbox, Crond, Pushd, createbuckets), minimum 2 vCPU / 2GB RAM. Self-hosted voice is **broken** (issue #138). Hearth advantage: 3 containers, 1 vCPU / 1GB, SQLite WAL, zero external dependencies. Strategic conclusion: not competing — Stoat is "Discord but open source," Hearth is "living room for 10 friends." Full analysis added to master plan §7.1. |
